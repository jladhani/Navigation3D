...
class PathFinder
{
	public:
		...
		*/
		...
		bool findPath(std::vector<TNode*>& solution, float AbortTime, bool& Aborted, int hint = 0 )
		{
			std::vector<typename TAlgorithm::node_type*> path;
			TAlgorithm* algorithm = TAlgorithm::create();

			if (hint > 0)
				path.reserve(hint);

			bool pathFound = algorithm->getPath(m_start, m_goal, path, AbortTime,Aborted);
			algorithm->clear();
			if(!pathFound)
				return false;
