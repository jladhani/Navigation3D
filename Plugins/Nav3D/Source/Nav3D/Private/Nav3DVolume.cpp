...
void ANav3DVolume::GetLeafOcclusionBoxes(TArray<FBoxCenterAndExtent>& out)
{
	...
	for (uint_fast32_t I = 0; I < static_cast<uint_fast32_t>(Octree.Leafs.Num()); I++) {
		for (uint8 J = 0; J < 64; J++) {
			if (Octree.Leafs[I].GetSubNode(J)) {
				...
				FVector NodeLocation;
				GetNodeLocation(Edge, NodeLocation);
				out.Add(FBoxCenterAndExtent(NodeLocation, FVector(VoxelHalfSizes[0] * 0.25f)));
			}
		}
	}
}
...
void ANav3DVolume::UpdateVolume() {
	...
	VolumeExtent = FVector(ActualVolumeSize/2.f);
	VolumeOrigin = GetActorLocation();
}


void ANav3DVolume::UpdateVolume2(UWorld* World) {
	// Calculate the nearest integer exponent to fit the voxel size perfectly within the volume extents
	VoxelExponent = FMath::RoundToInt(FMath::Log2(VolumeSize / (VoxelSize * 4)));
	NumLayers = VoxelExponent + 1;
...
void ANav3DVolume::RebuildDebugEdges()
{
	DebugEdges.Empty();
	for (int32 I = NumLayers - 2; I >= 0; I--) BuildEdges(I);
}
...
void ANav3DVolume::Serialize2(FArchive& Ar)
{
	Ar << Octree;
	Ar << VoxelHalfSizes;
	Ar << VolumeOrigin;
...
bool ANav3DVolume::IsOccluded(const FVector& Location, const float Size) const {
	...
	if (IsOccludedDel.IsBound())
	{
		FBox b = FBox(Location - FVector(Size + Clearance), Location + FVector(Size + Clearance));
		//bool result = IsOccludedDel.Execute(b);
		return IsOccludedDel.Execute(b);
	}
	else
	{
...
double ANav3DVolume::GetOpenVolume()
{
	TArray<double> VoxelVolumes;
	for (float halfSize : VoxelHalfSizes)
	{
		VoxelVolumes.Add(FMath::Pow(halfSize * 2.f, 3));
	}

	double SubNodeSize = FMath::Pow(VoxelHalfSizes[0] / 2.f, 3);
	NumLayers = static_cast<uint8>(Octree.Layers.Num());
	TArray<FNav3DOctreeEdge> Stack;
	Stack.Emplace(NumLayers-1, 0, 0);
	double TotalOpenVolume = 0;

	while (Stack.Num() > 0)
	{
		FNav3DOctreeEdge CurrentEdge = Stack.Pop(false);
		const FNav3DOctreeNode& CurrentNode = GetNode(CurrentEdge);

		if (CurrentEdge.LayerIndex == 0) {
			const FNav3DOctreeLeaf& leaf = Octree.Leafs[CurrentEdge.NodeIndex];
			if (leaf.IsEmpty())
			{
				TotalOpenVolume += VoxelVolumes[CurrentEdge.LayerIndex];
			}
			else if (!leaf.IsOccluded())
			{
				for (uint_fast64_t j = 0; j < 64; j++)
				{
					if (!leaf.GetSubNode(j)) TotalOpenVolume += SubNodeSize;
				}
			}
		}
		else
		{
			if (CurrentNode.HasChildren())
			{
				FNav3DOctreeEdge FirstChild = CurrentNode.FirstChild;
				for (size_t i = 0; i < 8; i++)
				{
					Stack.Emplace((int)FirstChild.LayerIndex, (int)FirstChild.NodeIndex+i, (int)0);
				}
			}
			else
			{
	...
	return TotalOpenVolume;
}


double ANav3DVolume::GetTotalVolume() const
{
	return FMath::Pow(VoxelHalfSizes.Last() * 2, 3);
}


int ANav3DVolume::GetConnections() const
{
	...
	while (Stack.Num() > 0)
	{
		FNav3DOctreeEdge CurrentEdge = Stack.Pop(false);
		const FNav3DOctreeNode& CurrentNode = GetNode(CurrentEdge);
		TArray< FNav3DOctreeEdge> Neigbours;
		if (CurrentEdge.LayerIndex > 0) {
			if (CurrentNode.HasChildren())
			{
				FNav3DOctreeEdge FirstChild = CurrentNode.FirstChild;
			...
			else
			{
				...
				GetAdjacentEdges(CurrentEdge, Neigbours);
			}
		}
		else if (CurrentEdge.LayerIndex == 0 && CurrentNode.FirstChild.IsValid()) {
			const FNav3DOctreeLeaf& Leaf = Octree.Leafs[CurrentNode.FirstChild.NodeIndex];
			FNav3DOctreeEdge LeafEdge = CurrentNode.FirstChild;
			for (size_t i = 0; i < 64; i++)
			{
				if (!Leaf.GetSubNode(i))
				{
					LeafEdge.SubNodeIndex = i;
					GetAdjacentLeafs(LeafEdge, Neigbours);
				}
			}
		}
		TotalConnections += Neigbours.Num();
...
int ANav3DVolume::GetNumNodes() const
{
	int TotalNumNodes = 0;
	for (const auto& layer : Octree.Layers)
	{
		TotalNumNodes += layer.Num();
	}
	TotalNumNodes += Octree.Leafs.Num() * 64;//64 nodes per leaf
