...
void UNav3DComponent::ExecutePathFinding2(
	ANav3DVolume* Volume,
	...
	G.Add(StartEdge, 0);
	F.Add(StartEdge, HeuristicScore(Volume,StartEdge, TargetEdge, Config));
	int32 I = 0;
	while (OpenSet.Num() > 0) {

		auto t2 = std::chrono::steady_clock::now();
		auto Duration = std::chrono::duration<float>(t2 - t1);
		if (std::chrono::duration_cast<std::chrono::milliseconds>(Duration).count() > AbortTime)
		{
			HasAborted = true;
			return;
		}
		...
		for (auto& AdjacentEdge : AdjacentEdges) {
			if (AdjacentEdge.IsValid()) {
				...
				F.Add(AdjacentEdge, G[AdjacentEdge] + Config.EstimateWeight * HeuristicScore(Volume,AdjacentEdge, TargetEdge, Config));
			}
		}
		I++;
	}
...
float UNav3DComponent::HeuristicScore(ANav3DVolume* Volume,const FNav3DOctreeEdge StartEdge, const FNav3DOctreeEdge TargetEdge, const FNav3DPathFindingConfig Config)
{
